...
const WorkSection = () => {
  ...
  return <section id="work" ref={sectionRef} className="py-32 relative overflow-hidden bg-secondary">
      <div className="container mx-auto px-6">
        <div ref={headerRef} className="flex flex-col md:flex-row md:items-end md:justify-between mb-16">
          <div>
            <span className="inline-block px-4 py-2 bg-secondary text-muted-foreground text-sm font-body rounded-full border border-border mb-6">Our Recent  Projects
          </span>
            <h2 className="font-display text-4xl md:text-5xl lg:text-6xl font-bold">
              Selected <span className="text-gradient">projects</span>
            </h2>
          </div>
          <a href="#contact" className="mt-6 md:mt-0 inline-flex items-center gap-2 text-primary font-body hover:underline">
            View all projects <ArrowUpRight className="w-4 h-4" />
          </a>
        </div>

        <div ref={projectsRef} className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {projects.map(project => <article key={project.title} className="group cursor-pointer" style={{
          perspective: "1000px"
        }}>
              <div className="relative overflow-hidden rounded-2xl mb-6">
                <img src={project.image} alt={project.title} className="w-full aspect-[4/3] object-cover transform group-hover:scale-105 transition-transform duration-500" />
